...
export const ProductsProvider = ({ children }) => {
  ...
  useEffect(() => {
    const cargarProductos = async () => {
      try {
        const respuesta = await fetch('https://68f69bb06b852b1d6f173af8.mockapi.io/api/productos');
  ...
  const editarProducto = async (productoActualizado) => {
    try {
      const respuesta = await fetch(`https://68f69bb06b852b1d6f173af8.mockapi.io/api/productos/${productoActualizado.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
